...
def compute_corr(prob_seq, next_seq, type_inputs, respID_seq, id_to_orgdict,  respID_to_orgDict, USE_dict):
...
def compute_mae(preds, labels):
    ...
    output_mae = mean_absolute_error(labels, preds)
    output_mse = mean_squared_error(labels, preds)
    return output_mse, output_mae
...
def computeTimeExp(time_seq, time_span):
...
def train(train_data, val_data, timespan,  model, optimizer, logger, saver, num_epochs, batch_size, grad_clip, id_to_orgDict, respid_to_orgDict, USE_dict):
    ...
    """
    ## for AL dataset, nn.CrossEntropyLoss --which is equivalent to sparse categorical crossentopry
    ...
    for epoch in range(num_epochs):
        ...
        for item_inputs, label_inputs, type_inputs, item_ids, timestamp, item_types, resp_ids, qresp_inputs, labels in train_batches:
            ...
            time = computeTimeExp(timestamp, timespan)
            ...
            ## for mae, mse need to detach grad. NO SIGMOID
            ...
            train_mse, train_mae = compute_mae(preds, labels)
            ...
            metrics.store({'mse/train': train_mse})
            metrics.store({'mae/train': train_mae})
        ...
        for item_inputs, label_inputs, type_inputs, item_ids, timestamp, item_types, resp_ids, qresp_inputs, labels in val_batches:
            ...
            val_mse, val_mae = compute_mae(preds, labels)
            metrics.store({'mse/val': val_mse, 'mae/val': val_mae})
        ...
        stop = saver.save(average_metrics['mae/val'], model)
        if stop:
            break
...
def do_prediction_ablation(args, ablation):
 ## ForSRL, the id_to_orgDict contains re-indexed Ques + SRL ID together. Need to change ID_to_orgDict if analyze srl dataset on only ques attempt values for baselines
    ...
    ## read df then calc qrep cos dataframes
    ...
    ### get_qresp_cos returns a dataframe including columns in input data plus a column with resp cos values
    ...
    ablation_fname = "  time_include = " + str(ablation["time_include"]) + "  qrelation_include = " + str(ablation["qrelation_include"]) + "  qrespCos_include = " + str(ablation["qrespCos_include"])
    ...
    saver = Saver_reg(args.savedir, param_str)
    ...
    for item_inputs, label_inputs, type_inputs, item_ids, timestamp, item_types, resp_ids, qresp_inputs, labels in test_batches:
        ...
        time = computeTimeExp(timestamp, args.timespan)
        with torch.no_grad():
            ...
            preds = (pred_2[labels_2 >= 0]).flatten().cpu().numpy()
    ...
    mse_test = mean_squared_error(correct, test_preds)
    mae_test = mean_absolute_error(correct, test_preds.round())

    print("mse_test = ", mse_test)
    print("mae_test  = ", mae_test )

    len_test =  "len arrays " + str(len(correct)) + " pred " + str(len(test_preds)) +  " batch count " + str(batch_count)
    mse_test_str = "mse_test = " + str(mse_test)
    mae_test_str = "mae_test = " + str(mae_test )

    logger.log_dowrite(len_test)
    logger.log_dowrite(mse_test_str)
    logger.log_dowrite(mae_test_str)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train SRL_KT.')
    parser.add_argument('--dataset', type=str)
    parser.add_argument('--logdir', type=str, default='./runs/SRL_Regression')
    parser.add_argument('--savedir', type=str, default='./save/SRL_Regression')
    ...
    parser.add_argument('--lr', type=float, default= 0.0005) #1e-3
